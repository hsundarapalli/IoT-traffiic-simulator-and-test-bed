...
def process_requests(request_queue, processed_queue, worker_id):
    while True:
...
def push_requests(request_queue, client_soc, client_id):
    while True:
...
def push_requests_to_queue(request_queue, server):
    ...
    while True:
        ...
        Thread(target=push_requests,
               args=(request_queue, client_soc, client_id,)).start()
...
def create_worker(worker_id):
    if pool_type == 1:
        Process(target=process_requests,
                args=(request_queue, processed_queue, worker_id)).start()
    else:
